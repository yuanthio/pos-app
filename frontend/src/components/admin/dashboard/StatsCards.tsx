...
import { Users, UserCheck, UserPlus, TrendingUp, Utensils } from 'lucide-react'
...
export function StatsCards({ stats }: StatsCardsProps) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Total Makanan</CardTitle>
          <Utensils className="h-4 w-4 text-muted-foreground" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{stats.total_makanan}</div>
          <p className="text-xs text-muted-foreground">Semua menu makanan</p>
        </CardContent>
      </Card>
    </div>
  )
